...
$Environment = Get-UserInput -Prompt "Enter environment (dev/staging/prod)" -Default "dev"
...
$SwaName = "swa-${ProjectName}-${Environment}-${Suffix}"
...
Write-Info "Customer UI calls Web BFF directly (CORS must be enabled on web-bff)"
...
try {
    ...
    # Build with BFF URL
    Write-Info "Building React app with REACT_APP_BFF_URL=$WebBffUrl"
    $env:REACT_APP_BFF_URL = $WebBffUrl
    npm run build
